...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class ProductController : BaseAdminController
    {
        ...
        public virtual async Task<IActionResult> Create(ProductModel model, bool continueEditing)
        {
            ...
            var existingProduct = await _productUnleashService.FindProductByNameAsync(model.Name);
            if (ModelState.IsValid && existingProduct is null)
            {
                ...
                if (currentVendor != null && model.ShowOnHomepage)
                 ...
                 /*
                //stripe
                //var createdProductId =  await CreateProductOnStripe(model);
                Stripe.Product createdProduct = await _productUnleashService.CreateProductOnStripe(model.Name);
                Stripe.Price priceObj = null;

                if (createdProduct is not null)
                    priceObj = await _productUnleashService.CreatePriceOnStripe(model.Price, createdProduct.Id);
                 */
                ...
                //// Commented Aqib's implementation
                ////product.UnleashGuid = guid;
                ////product.StripeProductId = createdProduct.Id;
                ...
                //insert into unleash
                //// Commented Aqib's implementation
                ////await CreateProductOnUnleash(model, product);
        ...
        public async Task<string>  GetSignature()
        {
            ...
            string privatekey = UnleashAPI.GetUnleashAPIKey();
        ...
        #region UnleashActivity
        [Route("FetchFromUnleash")]
        public async Task<string> GetProductFromUnleash()
        {
            var pageNumber = 1;
            var response = await _productUnleashService.FetchFromUnleash(pageNumber);
            ...
            while (pageNumber <= response.Pagination.NumberOfPages)
            {
                ...
                foreach (var item in response.Items)
                {
                    ...
                    product.FullDescription = item.ProductDescription;
                    product.Weight = Convert.ToDecimal(item.Weight);
                    product.Price = Convert.ToDecimal(item.CustomerSellPrice);
                    product.MinimumCustomerEnteredPrice = 0;
                    product.Width = Convert.ToDecimal(item.Width);
                    product.Height = Convert.ToDecimal(item.Height);
                    product.CreatedOnUtc = item.CreatedOn;
                    product.Sku = null;
                    //product.UnleashGuid = item.Guid;
                    ...
                    product.PreOrderAvailabilityStartDateTimeUtc = null;
                    ...
                    product.BasepriceBaseUnitId = 1; // 1 is for cart popup visibility
                    ...
                    //product.DownloadActivationType
                    //product.GiftCardType
                    //product.LowStockActivity
                    //product.ManageInventoryMethod
                    //product.RecurringCyclePeriod
                    //product.RentalPricePeriodId

                    var isProductExist = _productUnleashService.GetProductByUnleashGuidAsync(item.Guid);

                    if (isProductExist.Result is null)
                        await _productService.InsertProductAsync(product);

                    if (item.ProductGroup is not null)
                    {
                        //Add categories
                        var isCategoryExist = _categoryService.GetCategoryByUnleashProductGuidAsync(item.ProductGroup.Guid);

                        if (isCategoryExist.Result is null)
                        {
                            var categoryMapResponse = await _productUnleashService.InsertProductCategoryAsync(new Nop.Core.Domain.Catalog.Category
                            {
                                Name = item.ProductGroup.GroupName,
                                Description = string.Concat("<p>",item.ProductGroup.GroupName,"</p>"),
                                CategoryTemplateId = 1,
                                ParentCategoryId = 6,
                                UnleashProductGroupGuId = item.ProductGroup.Guid,
                                AllowCustomersToSelectPageSize = true,
                                CreatedOnUtc = DateTime.Now,
                                Published = true
                            });

                            await _productUnleashService.InsertProductCategoryMapping(new ProductCategory
                            {
                                ProductId = product.Id,
                                CategoryId = categoryMapResponse.Id,
                                DisplayOrder = displayOrder
                            });
                        }
                        displayOrder++;
        ...
        public async Task<string> CreateProductOnUnleash(ProductModel model , Nop.Core.Domain.Catalog.Product product)
        {
            CreateProduct obj = new CreateProduct();
            obj.ProductCode = product.Name;
            obj.ProductDescription = string.IsNullOrEmpty(product.ShortDescription) ? "description" : product.ShortDescription;
            obj.Barcode = null;
            obj.PackSize = null;
            ...
            obj.MinStockAlertLevel = Convert.ToString(product.MinStockQuantity);
            obj.MaxStockAlertLevel = null;
            ...
            obj.NeverDiminishing = false;
            obj.LastCost = 0;
            obj.DefaultPurchasePrice = 0;
            obj.DefaultSellPrice = 0;
            obj.AverageLandPrice = 0;
            obj.Obsolete = false;
            obj.XeroTaxCode = null;
            obj.XeroTaxRate = null;
            obj.TaxablePurchase = false;
            obj.TaxableSales = false;
            obj.XeroSalesTaxCode = null;
            obj.XeroSalesTaxRate = null;
            obj.IsComponent = false;
            obj.IsAssembledProduct = false;
            obj.CommerceCode = null;
            obj.CustomsDescription = null;
            obj.SupplementaryClassificationAbbreviation = null;
            obj.ICCCountryCode = null;
            obj.ICCCountryName = null;
            ...
            obj.XeroSalesAccount = null;
            obj.XeroCostOfGoodsAccount = null;
            obj.PurchaseAccount = null;
            obj.BinLocation = null;
            obj.Supplier = null;
            obj.AttributeSet = null;
            obj.SourceId = 0;
            obj.SourceVariantParentId = 0;
            obj.IsSerialized = false;
            obj.IsBatchTracked = false;
            obj.MinimumSellPrice = 0;
            obj.MinimumSaleQuantity = 1;
            obj.MinimumOrderQuantity = 1;
            obj.Guid = product.UnleashGuid;

            var response = await _productUnleashService.CreateProductOnUnleash(obj);
            return response;
        }

        //public  async Task<string> CreateProductOnStripe(ProductModel model)
        //{
        //    var productName = model.Name;
        //    Stripe.Product product = await _productUnleashService.CreateProductOnStripe(model.Name);
        //    return product.Id;
        //}
